...
for DataSetID = 4
    ...
    for model = 3
        ...
        path = sprintf('%s/Params_coupled.mat', sdir);
        if exist(path, 'file')
            load(path, 'y');
        else
            y = [y0(2:end) reshape([repmat(y0(1), 1, numel(T_i)); T_i], 1, 2*numel(T_i))];
        end
        
        % Determine start fit value
        y_old = y;
        d_old = fitPhageAttackCoupled(x, y_old, model, Time, BF, GFP, detectionThreshold, false);
         
        
        % FMINCON OR GENETIC ALGORITHM
        lb = [lb(2:end) reshape(repmat([0;  lb(1)], 1, numel(T_i)), 1, 2*numel(T_i))];
        ub = [ub(2:end) reshape(repmat([24; ub(1)], 1, numel(T_i)), 1, 2*numel(T_i))];
        ...
        % Call minimizier
        % FMINCON
%         options = optimoptions('fmincon', 'UseParallel', true, 'Algorithm', 'sqp');
%         y = fmincon(@(y)fitPhageAttackCoupled(x, y, model, Time, BF, GFP, detectionThreshold, true), y0, [], [], [], [], lb, ub, [], options);

        % GENETIC ALGORITHM
%         options = optimoptions('ga', 'UseParallel', true);
%         y = ga(@(y)fitPhageAttackCoupled(x, y, model, Time, BF, GFP, true), numel(y0), [], [], [], [], lb, ub, [], options);
        
        % FMINSEARCH
        options = optimset('MaxIter', 200*numel(y0));
        y = fminsearch(@(y)fitPhageAttackCoupled(x, y, model, Time, BF, GFP, detectionThreshold, true), y0, options);

        % Report change in conditions
        fprintf('Total parameter change: %.3f\n', norm(y-y_old))
        
        d_new = fitPhageAttackCoupled(x, y, model, Time, BF, GFP, detectionThreshold, false);
