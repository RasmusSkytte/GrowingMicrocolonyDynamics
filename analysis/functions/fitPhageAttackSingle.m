function dist = fitPhageAttackSingle(x, y, model, Time, BF, GFP, detectionThreshold, reportDistance)
...
if model == 3
    ...
    I = arrayfun(@(T)find(min(abs(t-(T-p.nu))) == abs(t-(T-p.nu)), 1), Time);
    R = R(I);
end
...
if reportDistance
    fprintf('\tfitting distance = %7.1f, y = [%s]\n', dist,  sprintf([repmat('%.3f, ', 1, numel(y)-1) '%.3f'], y) )
end
