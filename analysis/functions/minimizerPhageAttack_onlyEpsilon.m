...
path = '../fits/PhageAttackParams_onlyEpsilon.mat';
if exist(path, 'file')
...
y_old = y0;
d_old = fitPhageAttack(x, y_old, 4, Time, BF, GFP, false);

% Call minimizier
y = fminsearch(@(y)fitPhageAttack(x, y, 4, Time, BF, GFP, true), y0, optimset('MaxIter', 10*numel(y0)));

% Report change in conditions
fprintf('Total parameter change: %.3f\n', norm(y-y_old))

d_new = fitPhageAttack(x, y, 4, Time, BF, GFP, false);
